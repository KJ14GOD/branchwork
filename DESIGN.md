...
- **Set up workspace.** A bounded dialog, not a wizard: detected project type, the proposed setup command, proposed run commands, proposed verification commands, the **filenames** of required local files (never their contents), the **variable names** this project calls secret, and where the configuration will be written — the shared project file or this machine only. One primary action. Nothing runs until it is pressed. Local files are copied only after explicit consent, only when Git confirms they are ignored, and the dialog states plainly that Novus reads their names, not their values.
  - *Values only this machine has* (D-044): one row per declared secret name, with a StatusDot and the words **supplied on this machine** or **not supplied** — never characters, and never a mask, because a mask is still a length. Supply, Replace, and Forget are the only actions; nothing reads a value back, because no verb can. A value held for a name the project no longer declares says so and offers Forget. On a machine whose operating system has no credential storage the section says that plainly and offers nothing, and on any other machine it says a value is supplied where the workspace is hosted.
...
- **Runtime dock.** A bottom dock in the room, closed by default, sharing the room's width and never replacing the trace; below the single-column threshold it becomes a dedicated full-room view. One dock, four views, chosen by a switch at its left edge: **Terminal**, **Setup**, **Running**, **Verification** (D-045). This is where *detailed local output* lives, and the split is deliberate — the trace shows the milestone, the evidence panel shows the bounded result that supports a claim, and the same output never appears in all three.
  - *Terminal*: named tabs distinguish shell, run, test, and log, each showing running, failed, or stopped. Rendering is a real emulator, so colour, cursor addressing, alternate screens, and interactive programs work (D-046). Scrollback is bounded and local. It is available only on the machine hosting the workspace, and the room says why in words rather than hiding the control: controlling the mission is not unrestricted access to the host machine (D-042).
