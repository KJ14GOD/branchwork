...
- **Runtime dock.** A bottom dock in the room, closed by default, sharing the room's width and never replacing the trace; below the single-column threshold it becomes a dedicated full-room view. One dock, four views, chosen by a switch at its left edge: **Terminal**, **Setup**, **App**, **Checks** (D-045). The dock never says *Verification*: that word belongs to the evidence panel's ledger, and using it for a raw local log made one word mean a claim in one place and a file in another (D-047). This is where *detailed local output* lives, and the split is deliberate — the trace shows the milestone, the evidence panel shows the bounded result that supports a claim, and the same output never appears in all three.
  - *Terminal*: showing the dock opens a session — showing the terminal is the request. A tab is a glyph, its name, and a close; the kind and the running state were words nobody reads twice on the row with the least room (D-047). `+` adds one. Rendering is a real emulator, so colour, cursor addressing, alternate screens, and interactive programs work (D-046). Scrollback is bounded and local. It is available only on the machine hosting the workspace, and the room says why in words rather than hiding the control: controlling the mission is not unrestricted access to the host machine (D-042).
  - *Setup / Running / Verification*: the output of the commands the project declared, on the machine that ran them. Each names the command in mono, its state, and — when it ended — its exit code or the reason it stopped, by name: **timed out**, **cancelled**, **could not start**, or **exited {code}**. A process that has ended stays readable; "it failed and the output is gone" is the state this view exists to prevent. Where more than one run of a kind exists they list, newest first. Output is bounded and redacted before it is shown, and it never leaves the machine.
