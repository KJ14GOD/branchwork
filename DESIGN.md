...
- **Run.** One compact `Run ▾` control in the room's workspace controls, beside the Terminal and Evidence toggles — never a second navigation and never a toolbar. Its menu lists only what the project declared: the setup command, each named run command with the default first, and each named verification command. While a run command is alive the control collapses to the running command's name with Stop, and Open preview appears only when the project declared or Novus detected one. A command a participant cannot invoke renders disabled with the capability named. Suggested commands Novus inferred are marked as suggestions and never execute without confirmation.
- **Set up workspace.** A bounded dialog, not a wizard: detected project type, the proposed setup command, proposed run commands, proposed verification commands, the **filenames** of required local files (never their contents), and where the configuration will be written — the shared project file or this machine only. One primary action. Nothing runs until it is pressed. Local files are copied only after explicit consent, only when Git confirms they are ignored, and the dialog states plainly that Novus reads their names, not their values.
...
- **Terminal.** A bottom dock in the room, closed by default, sharing the room's width and never replacing the trace; below the single-column threshold it becomes a dedicated full-room view. Named tabs distinguish shell, run, test, and log, each showing running, failed, or stopped. Scrollback is bounded and local. It is available only on the machine hosting the workspace, and the room says why in words rather than hiding the control: controlling the mission is not unrestricted access to the host machine (D-042).
