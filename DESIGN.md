...
1. **Projects sidebar** — the persistent left rail (240px): the user's repositories (GitHub and local), each with a disclosure control that expands it to its missions/workstreams; nouns and counts only, plus Add project (GitHub picker or local folder) and Join with invitation. Disclosure and selection are separate acts — a project can be opened for reading without moving the room — and selecting a project always discloses it. An **attention view** at the rail's top surfaces Needs-you items across projects — the old Missions queue, demoted to a lens.
