...
2. **Open missions** — a window-level strip above the rail and the room, carrying the **working set**: only the missions this person has open (D-061). Not a second list — a project with nine missions and none open shows nothing here. A tab is the mission's name, its project's name when more than one project is open, and a close. `+` at the end opens a draft. It scrolls sideways when it overflows; the shell never does, and the selected tab is always scrolled into view.
