...
1. **Projects sidebar** — the persistent left rail (240px): the user's repositories (GitHub and local), each disclosing its **missions** when the row is pressed anywhere along its length — and this is the only place missions are listed (D-055) — the name is the control, not only the arrow beside it, and the arrow additionally discloses without moving the room; nouns and counts only, plus **Archived** at the foot with a count, present only when something has been filed away and opening a dialog rather than a list in the rail — putting a mission away and taking it back out must not be equally easy, or archival is just a second list (D-063) — **New mission** at the end of an open project's own list, and a restrained `+` on each repository row — revealed on hover *and* on keyboard focus, labelled `New mission in {repository}`, and never selecting or collapsing the row it sits on (D-061) — Add project (GitHub picker or local folder), and Join with invitation. Disclosure and selection are separate acts — a project can be opened for reading without moving the room — and selecting a project always discloses it. An **attention view** at the rail's top surfaces Needs-you items across projects — the old Missions queue, demoted to a lens.
...
- Projects: `⌘T` new **mission** in the selected project — the same act as the rail's New mission row — and `⌘1–9` its missions in rail order. Shipped, and stated here because a binding that exists and is undocumented is a binding nobody uses.
