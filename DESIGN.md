...
- **Evidence inspector.** Changes and Verification are two sections of one right-docked panel, with Overview as its first. It is a **full-height column against the window's right edge**, owning that corner of the shell: its own top row carries the ParticipantStack, who currently holds control, and the control that hides it. A single IconButton in the top bar shows it again — the one control that is always reachable regardless of which section was last read, carrying a tooltip and a label because it has no adjacent text. The room header keeps the state line and the baton; the section triggers live only in the panel, so the room is never a second navigation. Each section's trigger is also a toggle: pressing the section already showing closes the panel, and the active trigger reads as pressed. Sections: **Overview** (repository, mission branch, base ref and abbreviated SHA, branch status, runner and environment, participants and roles — all the machinery the header may not carry), **Changes**, **Verification**. A section renders only once its data exists; an empty fictional destination is prohibited pattern 11. Repository revision belongs to Overview, secondary to the mission.
