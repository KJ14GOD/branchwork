...
| An external harness's run leaves structured evidence | **Met** (2026-08-01) | `harness-evidence.test.ts` (12 tests, driven through `ClaudeCodeHarness.run` / `CodexHarness.run` over fake CLIs against real Git repositories, and through `POST /turns` + `GET /files` + `POST /cancel`). A Claude Code or Codex run now emits `harness.changes_observed` (git diff against the run's base, plus untracked files, `attributable: false` because the run shares the session's own tree), which folds into `RunProjection.filesChanged` and so into `GET /sessions/:id/files`; and writes a `receipt.created` through the same `buildReceipt` the built-in loop uses — cost as reported or null, never zero; tokens marked a floor via `callsMissingUsage`; `verification: unverified`, because no check crossed Novus however loudly the harness said it ran the suite. `POST /cancel` now reaches the subprocess (`SessionRegistry.liveHarnesses` → `Harness.stop`) and the run ends `run.cancelled` rather than as a failure. Confinement is pinned by tests that fail when the guard is removed: a session opened on a *subdirectory* of a checkout is refused outright (git reports paths relative to the larger root, which `isReportablePath` cannot detect) and says so on the timeline; `.env`/`.env.*` are never reported; a symlink out of the repository is named but never read. **Open:** (a) a harness that could not honour a cancellation is reported with `run.progress`, not `harness.unsupported`, whose `requested` enum has no `cancel` member; (b) an observed file enters `receipt.filesChanged` with `patches: 1`, which over-claims — `RunReceiptSchema` requires a positive integer there and no patch crossed Novus for any of these, so the honest record is the event's own `attributable` flag. Both need `packages/contracts`, which this slice did not hold |
