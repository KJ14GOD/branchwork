...
| Missions surface | Partial — replaced by the project-first shell (D-032): projects sidebar (GitHub + local, counts, needs-attention lens, add-project menu; selection is a background highlight only; an open project lists its workstreams inline), evidence `9-project-shell.png`. The old Missions list is gone; the attention lens is its successor |
