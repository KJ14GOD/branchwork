...
- **Controlling a mission is not owning the host machine** (D-042). `workspace.command` invokes commands *the project itself declared* and nothing else. An interactive shell on a workspace that lives on someone's laptop is never granted by the lease, by a role, or by anything the interface can offer; it belongs to the person whose machine it is.
