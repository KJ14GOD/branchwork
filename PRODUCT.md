...
- **Direction** (V0) — an attributed instruction submitted to a Session of a workstream and consumed by an execution, with the lifecycle in [Direction](#direction). A direction may carry **attached files** (D-150, D-151) — images and PDFs — Artifacts a person supplied rather than Novus captured, bounded in number and size, handed to the harness with the words.
