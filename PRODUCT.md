...
- **PullRequest** (V0) — a GitHub pull request the mission opened, created or adopted by Novus and tracked as mission state. A mission opens as many as its work earns (D-207) — one per decision, at most one open at a time per workstream — and keeps every one, merged or closed, as its publication story. A request somebody opened on the workstream's branch outside Novus — the agent with the host's own tools, a person in the browser — is **adopted** the first time Novus sees it (D-208): tracked in whatever state the host holds it, named as opened outside and by whom, with no decision behind it. Publishing a revision that an adopted request already merged is refused by name rather than sent to the host to refuse. **The work is followed onward (D-209)**: when a mission's request merged into a branch that is not the repository's default, a request carrying that branch to review — opened by anyone, anywhere — is adopted as **downstream** of the mission's own, because it is review of this mission's work one hop on; its reviewers' comments arrive in the room, and a person may answer them from here. The chain is followed hop by hop and ends where the work reaches the default branch. A downstream request is never the workstream's own publication: it blocks no Publish, answers for no decision, and is completed — readied, merged, closed — where it was opened, never from the room.
