...
Events (runner → control plane): workspace lifecycle transitions, the **declared command list** whenever the project's configuration says something different from what was last published, repository revision observations, checkpoint and sync outcomes, execution lifecycle transitions, harness activity summaries, `harness.usage` (what the harness says a turn cost — see [Harness protocol](#harness-protocol)), `boundary_reached`, `approval_required` (opaque harness payload passed through), file-change summaries, process start / readiness / exit, verification outcomes, heartbeats, gap markers. A harness activity event may carry the id of the tool call that produced it, which is how a harness's **own** subagent activity is grouped under the turn that spawned it (D-071); such activity never becomes a workstream, a branch, or a controller of its own. There is no heartbeat *verb*: a runner's liveness is a side effect of the authenticated polling it already does, and the control plane's sweep reads the last-seen time that polling maintains.
