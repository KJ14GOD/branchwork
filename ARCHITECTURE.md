...
| Execution | `exe_id`, `wst_id`, `session_id` (the Session this turn belongs to, never null), harness id, selected model and effort, `runner_id?`, `starting_direction_id?`, state, `started_by`, created/started/ended timestamps, `harness_session_id?`, `resumed_session`, exit outcome, failure reason, `last_origin_seq`, `latest_checkpoint_sha?`. Invariant: at most one non-terminal execution per workstream (partial unique index over the non-terminal states) — sessions do not weaken this: they take turns in the one workspace (D-083). |
