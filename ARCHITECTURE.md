...
Events (runner → control plane): workspace lifecycle transitions, the **declared command list** whenever the project's configuration says something different from what was last published, repository revision observations, checkpoint and sync outcomes, execution lifecycle transitions, harness activity summaries, `harness.usage` (what the harness says a turn cost — see [Harness protocol](#harness-protocol)), `boundary_reached`, `approval_required` (opaque harness payload passed through), file-change summaries, process start / readiness / exit, verification outcomes, heartbeats, gap markers. A harness activity event may carry the id of the tool call that produced it, which is how a harness's **own** subagent activity is grouped under the turn that spawned it (D-071); such activity never becomes a workstream, a branch, or a controller of its own. Since D-107 the join is complete in both directions: a `harness.tool` event also records the tool call's **own** id, so a worker's tagged events connect to the exact call that spawned them, and the spawning call's tool result — the one lifecycle fact the harness exposes about a worker's end — is recorded as `harness.worker.ended` with the result's own error flag and the worker's handed-back report. Both are recorded-and-nothing-else: no side effects, no state machine, no worker rows anywhere in the schema — a worker is a projection over these events, or it is nothing. There is no heartbeat *verb*: a runner's liveness is a side effect of the authenticated polling it already does, and the control plane's sweep reads the last-seen time that polling maintains. A *turn's* liveness is its own fact (D-114): the runner emits `execution.heartbeat` every few minutes while the harness process is alive, recorded-and-nothing-else — the stall watch never counts a pulse as progress, because a wedged process breathes too; what the pulse buys is the difference between "alive, saying nothing" and "nobody is there".
