...
| VerificationCheck | `chk_id`, `mission_id`, `exe_id?`, name, category (`test \| typecheck \| build \| lint \| diagnostic`), outcome (`passed \| failed \| skipped \| errored`), origin (`harness \| participant \| external`), `requested_by?`, the sanitized command, exit code, started/completed timestamps, duration, bounded redacted output, environment attribution, and the `checkpoint_sha` it proves. Harness-observed checks come from a structured tool call correlated with its own tool result (D-037); participant-run checks come from a `run_verification` command the server authorized. A check is **stale** when the workstream's head has moved past its `checkpoint_sha` — derived, never stored. |
...
Commands (control plane → runner): `provision_workspace`, `sync_repository`, `checkpoint_workspace`, `start_execution`, `apply_direction`, `pause`, `resume`, `stop`, `force_interrupt`, `run_setup`, `run_command`, `stop_command`, `run_verification`, `respond_approval` (the controller's typed answer to an `approval_required` event), `report_boundary_request` (transfer pending — ack at next safe boundary), `destroy_workspace`. All carry idempotency keys. The commands realized so far are `start_execution`, `apply_direction`, `stop_execution`, `report_boundary_request`, `run_setup`, `run_command`, `stop_command`, and `run_verification`; there is deliberately **no shell command** in the protocol, so remote interactive access to a local machine is structurally absent rather than merely unauthorized (D-042); the rest are protocol surface with no implementation yet. `sync_repository` carries the expected local and remote SHAs plus the authorizing lease id; `run_verification` executes checks defined in repository-level configuration; mission success criteria may add manual confirmations, recorded as VerificationChecks attributed to the confirming participant.
...
**Process supervision** lives in the runner, never the renderer. Setup commands are finite; run commands are long-lived; verification commands are finite and cancellable; terminals are PTY-backed. Every process is spawned into its own process group so stop, restart, and application shutdown reach the whole tree. Output is bounded and redacted before it can become shared evidence. Ports are allocated as a persisted per-workstream range so two workstreams on one host never collide. Nothing runs in the user's original checkout. After a relaunch the runner reports what is actually true — a recorded process is not presented as running unless the operating system still has it.
